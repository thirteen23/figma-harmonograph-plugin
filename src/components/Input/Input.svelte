...
function randomizeValue() {
  ...
  setValue(parseFloat(randomValue.toFixed(decimalPlaces)));
}
...
function handleKeyDown(event) {
  if (event.key === "ArrowUp" || event.key === "ArrowDown") {
    ...
    const newValue = parseFloat((currentValue + change).toFixed(decimalPlaces));
...
function setValue(newValue, updateInput = true) {
  ...
  value = newValue;
  if (updateInput) {
    inputValue = newValue.toFixed(decimalPlaces);
  }
  onValueChange(value);
}
</script>
